...
from iva.models import UnidadDeNegocio
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        file_path = 'tesoreria/management/commands/unidades_de_negocio_gestion.csv'
        with open(file_path, newline='', encoding='utf-8-sig') as csvfile:
            reader = csv.DictReader(csvfile, delimiter=';')
            for row in reader:
                try:
                    UnidadDeNegocio.objects.create(
                        unidad_de_negocio=row["unidad_de_negocio"],
                    ).save()
