...
interface EditarDesacopioProps {
    desacopio: Desacopio | null;
    open: boolean;
    onOpenChange: (open: boolean) => void;
    updateItem: (item: Desacopio) => void;
}
...
export function EditarDesacopio({ desacopio, open, onOpenChange, updateItem }: EditarDesacopioProps) {
    ...
    useEffect(() => {
        if (open && desacopio) {
            form.reset({
                fecha_entrega: desacopio.fecha_entrega,
                remito: desacopio.remito || '',
                nro_pedido: desacopio.nro_pedido || '',
                arquitecto: desacopio.arquitecto || '',
                articulo: desacopio.articulo,
                cantidad: desacopio.cantidad,
                unitario: desacopio.unitario,
                alicuota: desacopio.alicuota,
                obra: desacopio.obra,
                conciliado: desacopio.conciliado,
                acopio: desacopio.acopio,
                presupuesto_cliente_item: desacopio.presupuesto_cliente_item
            });
        }
    }, [open, desacopio, form]);
    ...
    const onSubmit = async (data: DesacopioFormData) => {
        ...
        try {
            const response = await patch_generico({model: 'acopios/desacopios-crud', id: desacopio.id, dif: data});
            toast.success('Desacopio actualizado correctamente');
            updateItem(response.data);
            onOpenChange(false);
        } catch (error) {
            console.error('Error updating desacopio:', error);
            toast.error('Error al actualizar el desacopio');
        }
    };

    if (!desacopio) return null;

    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh]">
                <DialogHeader>
                    <DialogTitle>Editar Desacopio</DialogTitle>
                </DialogHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <ComboboxAPI
                            ...
                            fieldToSend='id'
                            control={form.control}
                            initialLabel={`${desacopio.articulo_codigo} - ${desacopio.articulo_nombre}` || ''}
                        />
                        ...
                        <ComboboxAPI
                            ...
                            fieldToSend='id'
                            control={form.control}
                            initialLabel={desacopio.acopio_nombre || ''}
                        />
                        ...
                        <ComboboxAPI
                            ...
                            fieldToSend='id'
                            control={form.control}
                            initialLabel={desacopio.obra_nombre || ''}
                        />
