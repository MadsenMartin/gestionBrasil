...
import { get_mdo_vs_ppto_por_proveedor } from '@/endpoints/api'
...
export function MDOVSPPTOXProveedor() {
    ...
    const exportar_a_excel = async () => {
        setExportando(true)
        const status = await get_mdo_vs_ppto_por_proveedor(entidadSeleccionada, true)
        if (status === 200) {
            setSuccess(true)
            setTimeout(() => {
                setSuccess(false)
    ...
    return (
        <div className="mx-auto p-4 w-full">
            <div className="flex items-center mb-2">
                ...
                <h1 className="text-2xl font-bold mb-4">MDO VS PPTO por proveedor</h1>
            </div>
            <div className="flex flex-col md:flex-row gap-4 mb-4">
                <ComboboxAPIFormless
                    ...
                    model="proveedores"
                    fieldToSend={"id"}
                    fieldToShow={"nombre_fantasia_pila"}
                />
                <Button
